...
/** EDITABLE default guidance for the resume-parse prompt. */
export const RESUME_PARSE_DEFAULT_GUIDANCE = `You extract structured profile data from the raw text of a candidate's existing resume. You are populating a profile form, not writing or improving a resume.
...
const RESUME_PARSE_CONTRACT = `${UNTRUSTED_INPUT_POLICY}
...
Return ONLY valid JSON matching this exact shape, no markdown, no commentary:
{
  ...
  "linkedin": string,
  "summary": string,
  "educations": [ { "degree": string, "school": string, "startDate": string, "endDate": string, "graduationDate": string, "gpa": string } ],
...
export function buildResumeParseSystemPrompt(overrides?: PromptOverrides): string {
  const guidance = applyPromptPlaceholders(
    resolveGuidance(overrides, "resumeParse", RESUME_PARSE_DEFAULT_GUIDANCE)
  );
  return `${guidance}

${RESUME_PARSE_CONTRACT}`;
}
