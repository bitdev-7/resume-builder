-- Multiple resume profiles (personas) per account.
-- Each profile has its own name, title, contact, summary, and its own
-- education / experience / skills / certifications / projects. Account-level
-- settings (AI provider, alerts, prompt preferences) stay on `profiles`.
-- Run in Supabase Dashboard -> SQL Editor. Safe to re-run.

create extension if not exists pgcrypto;

-- ─────────────────────────────────────────────────────────────────────────
-- resume_profiles — one row per persona, many per account
-- ─────────────────────────────────────────────────────────────────────────
...
-- Backfill: one default resume profile per existing account, from its profile fields.
...
-- ─────────────────────────────────────────────────────────────────────────
-- Add profile_id to each content table (kept alongside user_id, which still
-- backs Row Level Security). Backfill to the account's default profile.
-- ─────────────────────────────────────────────────────────────────────────
...
-- Tag resume history with the profile it was generated from (optional, nullable).
